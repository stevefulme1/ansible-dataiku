...
def create_resource(client, params):
    ...
    return client.post("/api/v1/scenarios", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/api/v1/scenarios/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("scenario_id", ""))
    client.delete(f"/api/v1/scenarios/{resource_id}")
