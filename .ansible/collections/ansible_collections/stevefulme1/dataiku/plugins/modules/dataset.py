...
"""Ansible module for managing Dataiku DSS dataset resources."""
...
---
module: dataset
short_description: Manage DSS datasets
description:
    - Create, update, and delete Dataiku DSS dataset resources.
version_added: "1.0.0"
author:
...
options:
    ...
    dataset_name:
        description:
            - The dataset name for the Dataiku DSS resource.
        type: str
        required: true
    type:
        description:
    ...
    format_type:
        description:
            - The format type for the Dataiku DSS resource.
        type: str
    state:
        description:
...
def find_resource(client, params):
    ...
    try:
        result = client.get(f"/public/api/projects/{params['project_key']}/datasets/")
...
def create_resource(client, params):
    ...
    return client.post(f"/public/api/projects/{params['project_key']}/datasets/", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/public/api/projects/{params['project_key']}/datasets/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("dataset_id", ""))
    client.delete(f"/public/api/projects/{existing.get("projectKey", "")}/datasets/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['type', 'connection', 'format_type']
