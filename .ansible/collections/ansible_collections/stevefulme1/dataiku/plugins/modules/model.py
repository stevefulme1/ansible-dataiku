...
"""Ansible module for managing Dataiku DSS model resources."""
...
---
module: model
short_description: Manage DSS saved models
description:
    - Create, update, and delete Dataiku DSS model resources.
version_added: "1.0.0"
author:
...
options:
    ...
    analysis_id:
        description:
            - The analysis id for the Dataiku DSS resource.
        type: str
    state:
        description:
...
def find_resource(client, params):
    ...
    try:
        result = client.get(f"/public/api/projects/{params['project_key']}/savedmodels/")
...
def create_resource(client, params):
    ...
    return client.post(f"/public/api/projects/{params['project_key']}/savedmodels/", data=payload)
...
def update_resource(client, existing, params):
    ...
    return client.put(f"/public/api/projects/{params['project_key']}/savedmodels/{resource_id}", data=payload)


def delete_resource(client, existing):
    """Delete a resource."""
    resource_id = existing.get("id", existing.get("model_id", ""))
    client.delete(f"/public/api/projects/{existing.get("projectKey", "")}/savedmodels/{resource_id}")


def needs_update(params, existing):
    """Check if existing resource needs updating."""
    check_fields = ['analysis_id']
