...
"""Ansible module to query Dataiku DSS deployment resources."""
...
short_description: List or retrieve DSS deployments
description:
    - Retrieve information about Dataiku DSS deployment resources.
version_added: "1.0.0"
author:
...
- name: List all deployments
  stevefulme1.dataiku.deployment_info:
...
def main():
    ...
    if params.get("project_key"):
        result = client.get(f"/public/api/project-deployer/deployments/{params['project_key']}")
        module.exit_json(changed=False, deployments=[result] if result else [])
    else:
